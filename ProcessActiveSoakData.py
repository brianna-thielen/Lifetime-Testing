...
def main():
	...
	fig_impedance = make_subplots(
		...
		subplot_titles=(
			...
			"Average +/- SD (LCR, normalized to t=0)",
			"Average (Intan, normalized to t=0)"
		),
	...
	df_experiment, df_cvarea_smu, df_z_lcr, df_z_intan, df_cic500_scope, df_cic1000_intan = perform_data_analysis(args.path)

	dataframe_path = f"{args.path}//saved-dataframes//"
	df_experiment.to_csv(f"{dataframe_path}df_experiment.csv", index=False)
	df_cvarea_smu.to_csv(f"{dataframe_path}df_cvarea_smu.csv", index=False)
	df_z_lcr.to_csv(f"{dataframe_path}df_z_lcr.csv", index=False)
	df_z_intan.to_csv(f"{dataframe_path}df_z_intan.csv", index=False)
	df_cic500_scope.to_csv(f"{dataframe_path}df_cic500_scope.csv", index=False)
	df_cic1000_intan.to_csv(f"{dataframe_path}df_cic1000_intan.csv", index=False)

	plot_cv(fig_cv, df_experiment, f"{args.path}//manual-measurements")
	plot_cvarea(fig_cvarea, df_experiment, df_cvarea_smu)
	# plot_cv_day0(fig_cv_day0, args.path)

	plot_eis(fig_eis, df_experiment, f"{args.path}//manual-measurements")
	plot_impedance(fig_impedance, df_experiment, df_z_lcr, df_z_intan)
	# plot_eis_day0(fig_eis_day0, args.path)
...
def perform_data_analysis(path):
	...
	for folder in folders:
		# Note: folder is also the number of accelerated days

		# If it's the vt data, skip it, this was processed first
		# If it's the saved dataframes, skip it
		# If it's the archived measurements, skip it
		if folder == "Soak Testing - SIROF (Active).csv" or folder == "saved-dataframes" or folder == "archived-measurements":
			continue

		# If it's the intan test folder:
		elif folder == "intan-measurements":
			df_experiment, df_cic1000_intan, df_z_intan = process_intan_data(f"{path}//{folder}//", df_experiment, df_cic1000_intan, df_z_intan)
			
		# If it's the manual test folder:
		elif folder == "manual-measurements":
			df_experiment, df_cvarea_smu, df_z_lcr = process_manual_data(f"{path}//{folder}//", df_experiment, df_cvarea_smu, df_z_lcr)
...
def process_vt_spreadsheet_data(filepath, df_experiment, df_cic500_scope):
	...
	for idx, row in df.iterrows():
		...
		accel_days = float(row["Accel. Days (Total)"])
		temp = float(row["Temp\n(deg C)"])
		test_date = row["Date"]
		test_time = row["Time"]
...
def process_intan_data(path, df_experiment, df_cic1000_intan, df_z_intan):
	...
	for n in range(numfiles):
		...
		for i in range(len(df)):
			...
			if cic < 0:
				# print(f"Error: CIC < 0 ({cic}); file: {file}")
				cic = 0
			# elif cic > 1200:
				# print(f"Error: CIC > 1200 ({cic}); file: {file}")
...
def process_manual_data(path, df_experiment, df_cvarea_smu, df_z_lcr):
	print(f"Processing Manual CV and EIS data...")
	files = os.listdir(path)
	numfiles = len(files)
...
def plot_cv(fig_cv, df_experiment, path):
	...
	print(f"Plotting CV curves vs time...")
	files = os.listdir(path)
	numfiles = len(files)
...
def plot_cvarea(fig_cvarea, df_experiment, df_cvarea_smu):
	...
	print("Plotting CV area vs time...")
