...
DATA_PATH = "./data/LCP Encapsulation"
PLOT_PATH = "./data/Plots"
START_DATE = datetime.datetime(2024, 11, 4, 15, 25)

def process_encapsulation_soak_data(plot_on=False):
	"""
	Main function to process encapsulation soak test data.
	"""

	print("Processing LCP Encapsulation soak test data...")
	...
	fig_rhtemp.write_html(f"{PLOT_PATH}/LCP-Encapsulation_RH-vs-Time.html")

	# Show plot if requested
	if plot_on:
		fig_rhtemp.show()

	# # Extract most recent values for capacitive sensors
	# mask = df_encap.columns.str.contains("RH") & df_encap.columns.str.contains(r"\(C")
	# print(df_encap)
	# print(mask)
	# RH_cap_sensors = df_encap.loc[df_encap.index[-1], mask]
	# print(RH_cap_sensors)

	# # Extract most recent values for resistive sensors
	# mask = df_encap.columns.str.contains("RH") & df_encap.columns.str.contains(r"\(R")
	# RH_res_sensors = df_encap.loc[df_encap.index[-1], mask]

	# Extract most recent values for capacitive sensors
	c_cols = [col for col in df_encap.columns if "RH" in col and "(C" in col]
	RH_cap_sensors = [df_encap[col].dropna().iloc[-1] for col in c_cols]

	# Extract most recent values for resistive sensors
	r_cols = [col for col in df_encap.columns if "RH" in col and "(R" in col]
	RH_res_sensors = [df_encap[col].dropna().iloc[-1] for col in r_cols]

	# Extract days
	days = df_encap["Accel. Days (Total)"].iloc[-1]

	# df_encap.to_csv(f"{PLOT_PATH}/LCP_Encapsulation_Soak_Data.csv", index=False)

	return days, RH_cap_sensors, RH_res_sensors 
...
def process_spreadsheet_data(filepath):
	...
	for idx, row in df_encap.iterrows():
		date = row["Datetime"]
		days = (date - START_DATE).total_seconds() / 60 / 60 / 24
