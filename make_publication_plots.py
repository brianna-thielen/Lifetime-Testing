...
HUMIDITY_AMBIENT_COLOR = 'rgb(140,0,0)'
...
def plot_humidity_temp_one(groups, rh_range_25, rh_range_100, rh_range_amb, temp_range_ambient, temp_range_sample, t_range, vert_lines=False):
    # Setup plot
    fig_rh = make_subplots(
        rows=4,
        cols=1,
        row_heights=[SAMPLE_PLOT_HEIGHT, SAMPLE_PLOT_HEIGHT, SUPPLEMENT_PLOT_HEIGHT, SUPPLEMENT_PLOT_HEIGHT],
        # subplot_titles=(
        #     "A",
        #     "B",
        #     "C",
    ...
    fig_rh.update_yaxes(
        # title_text="RH (%)", 
        range=rh_range_25, row=1,
        showline=True, linewidth=2, linecolor='black', ticks='outside', tickcolor='black',tickwidth=2, gridcolor='lightgrey'
    )

    fig_rh.update_yaxes(
        # title_text="RH (%)", 
        range=rh_range_100, row=2,
        showline=True, linewidth=2, linecolor='black', ticks='outside', tickcolor='black',tickwidth=2, gridcolor='lightgrey'
    )

    fig_rh.update_yaxes(
        # title_text="Temp. (C)", 
        range=temp_range_sample, row=3, secondary_y=True,
        showline=True, linewidth=2, linecolor='black', ticks='outside', tickcolor='black',tickwidth=2, gridcolor='lightgrey'
    )

    fig_rh.update_yaxes(
        # title_text="RH (%)", 
        range=rh_range_amb, row=4, secondary_y=False, tick0=0, dtick=25,
        showline=True, linewidth=2, linecolor='black', ticks='outside', tickcolor='black',tickwidth=2, gridcolor='lightgrey'
    )

    fig_rh.update_yaxes(
        # title_text="Temp. (C)", 
        range=temp_range_ambient, row=4, secondary_y=True,
        showline=True, linewidth=2, linecolor='black', ticks='outside', tickcolor='black',tickwidth=2, gridcolor='lightgrey'
    )

    fig_rh.update_layout(showlegend=False)

    # Use C-100-1 for general temperature data
    df_temp_data = pd.read_csv(f"publication_plots/data/LCP Encapsulation Capacitive/ENCAP-C-100-1_data_summary.csv")
    temperature_general = df_temp_data["Temperature (C)"]
    real_days_general = df_temp_data["Real Days"]
    temperature_general = pd.DataFrame(temperature_general)
    real_days_general = pd.DataFrame(real_days_general)
    accel_days_general = calculate_accel_days(real_days_general, temperature_general)
    accel_days_general = [a / 365.25 for a in accel_days_general]  # convert to years
    ...
    for g, group in enumerate(groups):
        ...
        for s, sample in enumerate(group_info["samples"]):
            ...
            if "Ambient" in group:
                ...
                df = pd.merge_asof(
                    df,
                    df_temp_data[["Real Days", "Temperature (C)"]],
                    on="Real Days",
                    direction="nearest"
                )

                df = df.rename(columns={'Temperature (C)_y': 'Temperature (C)'})
                ...
                # Ambient humidity rolling average
                # rh = pd.Series(rh)
    ...
    fig_rh.add_trace(
        go.Scatter(
            x=accel_days_general,
            y=temperature_general["Temperature (C)"],
            mode="lines",
    ...
    # Fake plot to make plotly not crash w/o primary axis data
    fig_rh.add_trace(
        go.Scatter(
            x=accel_days,
            y=[None] * len(accel_days),  # or zeros
            showlegend=False,
            hoverinfo="skip",
            line=dict(width=0),
        ),
    ...
    fig_rh.add_trace(
        go.Scatter(
            x=accel_days_general,
            y=temperature_ambient["Temperature (C)"],
            mode="lines",
            line=dict(width=TEMPERATURE_LINE_WIDTH, color=TEMPERATURE_COLOR, dash=TEMPERATURE_LINE_SOLID),
            name="Sample Temperature",
            connectgaps=False
...
def plot_ide_good_one(groups, deltaz_range, temp_range, t_range, bad_samples, vert_lines=False):
    # Setup plot
    fig_z = make_subplots(
        rows=4,
        cols=1,
        ...
        for s, sample in enumerate(group_info["samples"]):
            ...
            fig_z.add_trace(
                go.Scatter(
                    ...
                    y=z_norm,
                    mode="lines",
                    line=dict(width=IDE_SAMPLE_LINE_WIDTH, color=line_color, dash=IDE_SAMPLE_SOLID),
                    name=sample,
                    # showlegend=True
                ),
                row=row,
            ...
            if vert_lines:
                ...
                for flag_g, dates_g in flagged_dates_group.items():
                    ...
                    for date_g in dates_g:
                        ...
                        add_vert_line(fig_z, 1, 2, accel_days_flag, f"{flag_g} ({sample} @ {round(accel_days_flag)} days)", 'black', True)
...
#     )
#     fig_rh.update_yaxes(
#         title_text="Temperature (C)", range=[45, max_temp], row=2,
#         showline=True, linewidth=2, linecolor='black', ticks='outside', tickcolor='black',tickwidth=2, gridcolor='lightgrey'
#     )
