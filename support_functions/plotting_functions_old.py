...
def plot_cic(groups, data_path, sample_info_path, plot_path, title, plot_norm=False, plot_flags=False):
    ...
    fig_cic.update_yaxes(
            title_text="Charge Injection Capacity Change (uC/cm<sup>2</sup>)", 
            col=1,
            showline=True, linewidth=2, linecolor='black', ticks='outside', tickcolor='black',tickwidth=2, gridcolor='lightgrey'
        )
    ...
    for g, group in enumerate(groups):
        ...
        for s, sample in enumerate(group_info["samples"]):
            ...
            real_weeks = [day/7 for day in real_days["Real Days"]]
        ...
        fig_cic.add_trace(
            go.Scatter(
                # x=accel_days, #This will take accelerated days from the last sample - if they all use the same temp reading this is fine
                x=real_weeks,
                y=cic_average,
                mode="lines",
                line=dict(width=1, color=shade),
    ...
    # Update plot title
    fig_cic.update_layout(title_text=f"{title} - {total_real_weeks} weeks at average {avg_temp} \u00b0C, equivalent to {total_accel_weeks} weeks at 37 \u00b0C")
